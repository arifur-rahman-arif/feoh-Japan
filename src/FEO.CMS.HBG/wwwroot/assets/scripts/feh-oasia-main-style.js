...
$(document).ready(function() {
  if ($('.header_ribbon').length) {
    ...
    $('header #choose_oasia-1').on("change",function(){
        if ($('header #choose_oasia-1 option:selected').attr("data-propertynote")) {      
            var propertytext1 = $("header #choose_oasia-1 option:selected").attr('data-propertynote');
            $('header .shownote').text(propertytext1).show();
          }
          else
          {
            $('header .shownote').hide();
          }
    });
    $('.heroes-booking #choose_oasia-1').on("change",function(){
        if ($('.heroes-booking #choose_oasia-1 option:selected').attr("data-propertynote")) {      
            var propertytext2 = $(".heroes-booking #choose_oasia-1 option:selected").attr('data-propertynote');
            $('.heroes-booking .shownote').text(propertytext2).show();
          }
          else
          {
            $('.heroes-booking .shownote').hide();
          }
    });
    $('.booking-widget-mobile #choose_oasia-1-mobile').on("change",function(){
        if ($('.booking-widget-mobile #choose_oasia-1-mobile option:selected').attr("data-propertynote")) {      
            var propertytext2 = $(".booking-widget-mobile #choose_oasia-1-mobile option:selected").attr('data-propertynote');
            $('.booking-widget-mobile .shownote').text(propertytext2).show();
          }
          else
          {
            $('.booking-widget-mobile .shownote').hide();
          }
    });
    //Added booking widget popup close button
    ...
    $(".plus-sign").on('click', function(e) {
            ...
            if(max_limit == (totalsum+1)){
                element.parents(".dropdown-rooms").find(".add-less-icon.plus-sign").addClass("active");
            ...
            element.parents(".adult-rooms-child-section").find("text."+mapelementid).html(currentnumber+1);
            element.parents("form").find("input[name='"+mapelementid+"']").val(currentnumber+1);
            //$.cookie("cookie-"+mapelementid,currentnumber+1, { path: '/' });
    });

    $(".best-rate-page .file-control input[type='file']").on('change', function(){
      ...
      if(typeof filename !== 'undefined' && filename.length > 0){
        $(this).parent().find('label.file-control-proxy').text(filename);
      }else{
        $(this).parent().find('label.file-control-proxy').html("<span>No file chosen <br> (JPG, PNG, Max Size: 2mb)</span>");
      }
    });
    ...
    $(".minus-sign").on('click', function(e) {
            ...
            if(min_limit==number-1){
                element.addClass("active");
                 element.parents(".dropdown-rooms").find(".add-less-icon.plus-sign").removeClass("active");
            ...
            element.parents(".adult-rooms-child-section").find("text."+mapelementid).html(number-1);
    ...
    checkforMobileDevice();
      ...
      var nextArrowIcon = "<div class='brands-carousel-slider-arrow slider-arrow-next' id='tag-carousel-left-button'></div>";
      var prevArrowIcon = "<div class='brands-carousel-slider-arrow slider-arrow-prev' id='tag-carousel-right-button'></div>";
     ...
     $('.brands-carousel').slick({
       ...
       /*if(index === 4) {
        $(this).find(".slider-arrow-next").hide();
           $(this).find(".slider-arrow-prev").hide();
       }
       else {
...
$('.brands-carousel').find(".slider-arrow-prev").hide();
var brandcarousellength = $('.brands-carousel .slick-slide').length;
 $('.brands-carousel').on('afterChange', function(event, slick, currentSlide, nextSlide) {
     
                var totalcount=0;
                if($(window).width()>1000){
                    totalcount=currentSlide+4;
                }else if($(window).width()<=1000 && $(window).width()>600){
                    totalcount=currentSlide+3;
                }else{
                    totalcount=currentSlide+2;
                }
                if (currentSlide == 0) {
            $(this).find(".slider-arrow-next").show();
            $(this).find(".slider-arrow-prev").hide();
        } else if (totalcount == brandcarousellength) {
            $(this).find(".slider-arrow-next").hide();
            $(this).find(".slider-arrow-prev").show();
        } else {
            $(this).find(".slider-arrow-next").show();
            $(this).find(".slider-arrow-prev").show();
        }
     ...
     $('.affiliate-brands-carousel').slick({
        slidesToScroll: 1,
        infinite: false,
         speed: 300,
         dots: false,
...
$(window).scroll(function() {
 ...
 $(document).mouseup(function(e) {
  ...
  var enquiryLabel = $('<option />').text($('.enquiry-type label').text()).addClass('bs-title-option');
  $('.enquiry-type .app-select-control').prepend(enquiryLabel);
  var propertyLabel = $('<option />').text($('.property-type label').text()).addClass('bs-title-option');
  $('.property-type .app-select-control').prepend(propertyLabel);

  // $('.oasia-requestquote .datepicker-control .sr-in').on('change', function(){
  //   var arrivalDate = $(this).val();
  //   var d = new Date(arrivalDate);
  //   d.setDate(d.getDate() + 1);
  //   $('.oasia-requestquote .datepicker-control .sr-out').attr('min',d.toISOString().split('T')[0]);
  // });
  ...
  $(".facebook-app-embed-link").on("click",function(e){
    ...
    setTimeout(function () {
        var navTopInitPosition = $('.main-header').css('top');
        var navTopMobileInitPosition = $('.main-header-mobile').css('top');
        var bodyInitPosition = $('.main-content').css('margin-top');
        ...
        $('.promoData_toggler').on('click',function(e) {
            ...
            if ($('.header_ribbon__promoData_detail').is(":hidden")) {
              var navTop = parseInt($('.main-header').css('top')) + ribHeight;
              var navMobileTop = parseInt($('.main-header-mobile').css('top')) + ribHeight;
              var bodyMargin = parseInt($('.main-content').css('margin-top')) + ribHeight;

              $('.main-header').attr('style', 'top:' + navTop + 'px !important');
              $('.main-header-mobile').attr('style', 'top:' + navMobileTop + 'px !important');
              $('.main-content').attr('style', 'margin-top:' + bodyMargin + 'px !important');
            } else {
                $('.main-header').attr('style', 'top:' + navTopInitPosition + ' !important');
                $('.main-header-mobile').attr('style', 'top:' + navTopMobileInitPosition + ' !important');
                $('.main-content').attr('style', 'margin-top:' + bodyInitPosition + ' !important');
            }
...
$(document).ready(function () {
  ...
  var sign_img_gif = '<img src="/content/images/oasia/loading-ohw-black.gif" width="16" height="16">';
