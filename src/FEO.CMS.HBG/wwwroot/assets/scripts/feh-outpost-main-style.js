...
function onSignoutClick () {
    $('body').on('click', function(e) {
        if (!$(e.target).closest(".modal-content").length && $('body').hasClass('signup-modal-open')) {
...
$(document).ready(function() {
   ...
   var logoLength = $('.brands-carousel li').length;
    ...
    /* $(".venues-items-carousel .owl-nav .owl-prev").css("display", "none");
    owlVenuesItemsCarousel.on('changed.owl.carousel', function(event) {
        var items = event.item.count; // Number of items
        var item = (event.item.index + 1) * event.page.size;

        if (event.item.index == 0) {
            $('.venues-items-carousel .owl-nav .owl-next').show();
            $('.venues-items-carousel .owl-nav .owl-prev').hide();
        } else if (item >= items) {
            $('.venues-items-carousel .owl-nav .owl-next').hide();
            $('.venues-items-carousel .owl-nav .owl-prev').show();
        } else {
            $('.venues-items-carousel .owl-nav .owl-next').show();
            $('.venues-items-carousel .owl-nav .owl-prev').show();
        }
...
$(document).ready(function () {
    ...
    var sign_img_gif = '<img src="/content/images/outpost/loading-toh-black.gif" width="16" height="16">';
                    ...
                    success: function(response) {
                        ...
                        if(response.indexOf("<script") === -1){
                           ...
                           $('input[name=__RequestVerificationToken]').val(token);
                            var message = $(response).find('.validation-summary-errors ul li.list-group-item').html();
                            $(".g-recaptcha").siblings(".field-validation-valid.help-block").html(message);
